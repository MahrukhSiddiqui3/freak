...
if(isset($_POST["submit_key"])&& $_POST['submit_key'] == 'submit_value')
{
	...
	if($msg == "")
	{
		 ...
		 if($ImageFile!='')
		 {
			 ...
			 $ext = end($ext);
			 $ImageFile = 'P-'.$ProductID.'.'.$ext;
			 $moved = move_uploaded_file($_FILES['ImageFile']['tmp_name'],DIR_UPLOADING_PRODUCTS.$ImageFile);
			 if($moved)
			 {
				 $sql = "UPDATE products SET Image = '".$ImageFile."' WHERE ID = ".$ProductID;
		 ...
		 $res = mysql_query($sql) or die(mysql_error());
			 $row = mysql_fetch_assoc($res);
			 if ($row['Subimages']=='' && ($_FILES['ImagesFile']['name'][0] =="" && count($_FILES['ImagesFile']['name'])==1))
			 {
				 $msg = '<div class="callout callout-danger">
			...
			$imagefile = array_filter($imagefile); //array filter removes any extra space in array
			 ...
			 for($i=0;$i<$countofimages;$i++)
			 {
				 ...
				 { 
					 ...
					 $moved1 = move_uploaded_file($_FILES['ImagesFile']['tmp_name'][$i],DIR_UPLOADING_PRODUCTS.$SubImages);
			  ...
			  <div class="row">
			   ...
			   <div class="form-group">
				...
				<?php
					...
					{
				?>
					<input type="checkbox" name="CatIDs[]" value="<?php echo $row['ID']; ?>" <?php echo(in_array($row['ID'],$CatIDs) ? 'checked' : '' ); ?> />&nbsp;&nbsp;<?php echo $row['CategoryName'];?>
					<br>
					<?php
					}
					?>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-10">
						<img style="width:150px;" src="<?php echo DIR_UPLOADING_PRODUCTS.$ImageFile?>"/>
                  </div>
                </div>
                <div class="form-group">

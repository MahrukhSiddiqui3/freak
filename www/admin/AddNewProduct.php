...
{
	// print_r($_FILES);
	// echo $_FILES['ImagesFile']['name'][0];
	// exit();
	if(isset($_POST["Status"]) && ($_POST["Status"]==1 || $_POST["Status"]==0))
	        $Status=trim($_POST["Status"]);
	if(isset($_POST["TopFeaturedID"]) && ($_POST["TopFeaturedID"]==1 || $_POST["TopFeaturedID"]==0))
	        $TopFeaturedID=trim($_POST["TopFeaturedID"]);
	...
	if(isset($_FILES['ImagesFile']))
	        $countofimages=count($_FILES['ImagesFile']['name']);
	// echo $countofimages;
	// exit();
	if($ImageFile=="")
	{
    ...
    else if($countofimages==0 || ($_FILES['ImagesFile']['name'][0] == '' && count($_FILES['ImagesFile']['name'])==1))
	{
		 $msg ='<div class="callout callout-danger">
	...
	if($msg == "")
	{
		$CatIDs = implode(",",$CatIDs);
		 $query = "INSERT INTO products SET 
				Status = '".(int)$Status."',
				TopFeaturedID = '".(int)$TopFeaturedID."',
				CatIDs = '".$CatIDs."',
		 ...
		 if($ImageFile!='')
		 {
			 ...
			 $ext = end($ext);
			 $ImageFile = 'P-'.$lastid.'.'.$ext;
			 $moved = move_uploaded_file($_FILES['ImageFile']['tmp_name'],DIR_UPLOADING_PRODUCTS.$ImageFile);
			 if($moved)
			 {
				 $sql = "UPDATE products SET Image = '".$ImageFile."' WHERE ID = ".$lastid;
			 ...
			 {
				 if($_FILES['ImagesFile']['name'][$i]!='')
				 {
					 $ext = explode(".",$_FILES['ImagesFile']['name'][$i]);
					 $ext = end($ext);
					 $ImageFile = 'P-'.$lastid.'-'.$numofimage++.'.'.$ext;
					 $moved1 = move_uploaded_file($_FILES['ImagesFile']['tmp_name'][$i],DIR_UPLOADING_PRODUCTS.$ImageFile);
					 array_push($arrayofimage,$ImageFile);
  ...
  <script>
	$(document).ready(function(){
			$('#btnaddmore').click(function(){
				//$("#imagespanel").append('<br><input type="file" name="ImagesFile[]" id="ImageFile">');
				$("#imagespanel").append('<div class="form-group divdel"><div class="col-sm-12" id="imagespanel1"><input type="file" name="ImagesFile[]" id="ImagesFile"></div></div>');
			});
			   ...
			   <div class="form-group">
				...
				<label for="inputEmail3" class="col-sm-2 control-label">Top Featured</label>
                  ...
                  <div class="col-sm-10" style="padding-top: 8px;">
                        ...
                        <input type="checkbox" value="1" <?php echo($TopFeaturedID == 1 ? 'checked' : ''); ?> name="TopFeaturedID"> Enable
				...
				<?php
					...
					{
				?>
					<input type="checkbox" class="chkIds" name="CatIDs[]" value="<?php echo $row['ID']; ?>" <?php echo(in_array($row['ID'],$CatIDs) ? 'checked' : '' ); ?> />&nbsp;&nbsp;<?php echo $row['CategoryName'];?>
					<br>
					<?php
					}
					?>
				...
				<div class="col-md-6">
				  ...
				  <input type="file" name="ImagesFile[]" id="ImageFile">
                  <br>
                  </div>
                </div>
				</div>

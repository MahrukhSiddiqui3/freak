...
if(isset($_REQUEST["CategoryID"]) && ctype_digit($_REQUEST["CategoryID"]))
...
if(isset($_POST["submit_key"])&& $_POST['submit_key'] == 'submit_value')
{
	...
	if($msg == "")
	{
		...
		if($ImageFile!='')
		{
			...
			$ext = end($ext);
			$ImageFile = 'C-'.$CategoryID.'.'.$ext;
			$moved = move_uploaded_file($_FILES['ImageFile']['tmp_name'],DIR_UPLOADING_CATEGORIES.$ImageFile);
			if($moved)
			{
				$sql = "UPDATE categories SET Image = '".$ImageFile."' WHERE ID = ".$CategoryID;
				mysql_query($sql) or die(mysql_error());
			}
		}
...
{
	$sql ="SELECT ID,ParentID,CategoryName,Image,Description,Status FROM categories WHERE ID='".$CategoryID."'";
      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="box box-info">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-horizontal" action="<?php echo $self; ?>?CategoryID=<?php echo $CategoryID;?>" method="post" enctype="multipart/form-data">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
						<img style="width:150px;" src="<?php echo DIR_UPLOADING_CATEGORIES.$ImageFile?>"/>
                  </div>
                </div>
			    ...
			    <select class="form-control" name="ParentID">
					...
					<?php
					$sql = "SELECT ID,CategoryName FROM categories WHERE ID <> ".$CategoryID;
					$res = mysql_query($sql) or die(mysql_error());
